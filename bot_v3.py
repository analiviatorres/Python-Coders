...
url_webhook = "hp" #endereço que o aplicativo do ngrok disponibiliza após ser executado no computador(obs: esse endereço muda todas as vezes que programa é executado.).
...
def process_message(dados):
    ...
    if text_received == "/start": # A palavra "/start" será o gatilho da interação.
        welcome_message = ("Bem-vindo ao bot Scooby-doo! 🤖\n"
                           ...
                           "Digite '/ip' para eu te mostrar qual é o seu IP! "
        ...
        user_ip = get_user_ip()
        response_message = f"Seu endereço IP é: {user_ip}"
        send_message(chat_id, response_message)
...
def get_user_ip():
    try:
        # Cria um socket e obtém o nome do host local.
