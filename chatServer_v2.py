...
url_webhook = "https://636f-187-19-156-21.ngrok-free.app" #endereço que o aplicativo do ngrok disponibiliza após ser executado no computador(obs: esse endereço muda todas as vezes que programa é executado).
...
# Lógica para processar mensagens recebidas do bot do Telegram.
def process_message(dados):
    ...
    #Abaixo uma serie de condições para cada solicitação do usuário.
        ...
        response_message = "Comando não reconhecido. Digite '/start' para ver os comandos disponíveis ou '/sair' para encerrar o chat."
        send_message(chat_id, response_message)
...
def receber_mensagem():
    ...
    resposta = {'resposta': 'Mensagem recebida com sucesso!'}
    
    enviar_resposta_cliente(mensagem)

    return jsonify(resposta)

@app.route('/receber_resposta', methods=['GET'])
def enviar_resposta_cliente():
    resposta = {'mensagem': 'Esta é uma resposta do servidor.'}
    return jsonify(resposta)
...
if __name__ == '__main__': #essa condição verifica se o script está sendo executado.
    ...
    app.run(port=5000, debug=True) # Inicia o servidor na porta 5000.
